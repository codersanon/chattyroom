...
function checkGame() {
	if (cClicks == 2) {
		...
		//Change Styles
		document.getElementById("gamestyle").innerHTML = '#main{cursor:pointer;}#main:hover{transform:scale(1.05);opacity:0.7;}#main:active{border-width: 20px;border-color:black;}.smalltext{font-size:13px; opacity: 0.7;margin:0;}.fakemessage{max-width:300px; padding-bottom:20px;opacity:0.8; position:fixed; display:block;transition:top 6500ms, opacity 500ms;border-radius:5px 20px 20px 5px;border:1px solid black;}'
...
function GenerateFakeMessage(sender) {
	...
	if (Math.floor(Math.random() * 100) > 50) {
		...
		//Position And Message Randomly Calculated
		var randomMessage = Math.floor(Math.random() * (randomMessageArray.length));
		var randomtop = Math.floor(Math.random() * 550 + 500) + "px";
		var randomleft = Math.floor(Math.random() * window.innerWidth - 80) + "px";
		var randomzindex = Math.floor(Math.random() * 10 - 20);
		//console.log(randomMessage + " " + randomleft + " " + randomtop + " " + sender)
		...
		newElement.innerHTML = "<div class='name'>" + sender + "</div><p class='text'>" + randomMessageArray[randomMessage]; + "</p>"

		//Make It Appear And Move Upwards
		newElement.style.opacity = 0;
		p.appendChild(newElement);
		window.setTimeout(function () {
			newElement.style.top = "-800px";
